...
def inspect_single_video_quick(video_file: VideoFile, ffmpeg_cmd: str, verbose: bool = False) -> VideoInspectionResult:
    ...
    try:
        ...
        logger.error(f"Quick scan failed for {video_file.filename}: {e}")
...
def inspect_single_video_deep(video_file: VideoFile, ffmpeg_cmd: str, verbose: bool = False) -> VideoInspectionResult:
    ...
    try:
        ...
        logger.error(f"Deep scan timed out for {video_file.filename}")
    except Exception as e:
        result.error_message = f"Deep scan failed: {str(e)}"
        logger.error(f"Deep scan failed for {video_file.filename}: {e}")
...
def inspect_video_files_cli(directory: str, resume: bool = True, verbose: bool = False, 
    ...
    try:
                ...
                for future in future_to_index:
                    try:
                        ...
                        if deep_result.is_corrupt:
                            corrupt_count += 1
                            logger.warning(f"Deep scan confirmed corruption: {deep_result.filename}")
                            if not verbose:
        ...
        logger.warning(f"Scan interrupted by user after processing {processed_count}/{total_files} files")
    ...
    logger.info(f"Scan summary - Total: {processed_count}, Corrupt: {corrupt_count}, "
    ...
    if json_output:
        ...
        try:
            ...
            logger.error(f"Could not save JSON results: {e}")
