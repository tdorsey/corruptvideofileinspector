...
def sync_to_trakt_watchlist(
    ...
    try:
        ...
        logger.error(f"Failed to initialize Trakt API: {e}")
        raise
    ...
    try:
        ...
        logger.error(f"Failed to process scan file: {e}")
        raise
    ...
    for i, media_item in enumerate(media_items, 1):
        if verbose:
            ...
            print(f"  {progress} Processing: {media_item.title} ({media_item.year}) [{media_item.media_type}]")
    ...
    if verbose or logger.isEnabledFor(logging.INFO):
        ...
        print(f"Success rate: {((summary['movies_added'] + summary['shows_added']) / summary['total'] * 100):.1f}%")
    
    logger.info(f"Trakt sync completed: {summary['movies_added']} movies, {summary['shows_added']} shows added")
