...
class TestCorruptionDetector:
    ...
    def test_analyze_critical_exit_codes(self):
        ...
        # In quick scan, critical exit codes should trigger deep scan
        analysis = detector.analyze_ffmpeg_output(normal_output, exit_code=69, is_quick_scan=True)
        assert analysis.needs_deep_scan
        assert "code 69" in analysis.error_message

        # In deep scan, critical exit codes should indicate corruption
        analysis = detector.analyze_ffmpeg_output(normal_output, exit_code=69, is_quick_scan=False)
        assert analysis.is_corrupt
        assert "code 69" in analysis.error_message
