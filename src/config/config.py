...
Supports configuration loading from environment variables and Docker secrets
...
class Config:
    ...
    trakt: TraktConfig = field(default_factory=lambda: TraktConfig(client_id=""))
...
class ConfigLoader:
    ...
    def load_from_environment(self: "ConfigLoader") -> None:
    ...
    def load_docker_secrets(self: "ConfigLoader") -> None:
        """Load configuration from Docker secrets."""
        secrets_path: Path = Path(self.config.secrets.docker_secrets_path)
        ...
        for secret_name, (section, key) in secret_mappings.items():
            secret_file: Path = secrets_path / secret_name
            if secret_file.exists():
                try:
                    value: str = secret_file.read_text().strip()
                    self._set_config_value(section, key, value)
                    logger.debug(f"Set {section}.{key} from Docker secret: " f"{secret_name}")
                except Exception as e:
                    logger.warning(f"Failed to read secret {secret_name}: {e}")

    def _apply_config_dict(self: "ConfigLoader", data: Dict) -> None:
    ...
    def _set_config_value(self: "ConfigLoader", section: str, key: str, value: object) -> None:
    ...
    def get_loaded_files(self: "ConfigLoader") -> List[str]:
...
def load_config() -> Config:
    ...
    Args:
        config_file (Optional[Union[str, Path]]): Path to config file (unused).
