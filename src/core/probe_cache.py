...
class ProbeResultCache:
    ...
    def get(self, file_path: Path) -> Optional[ProbeResult]:
        """
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            expired_count = sum(
                ...
                if result.is_expired(self.max_age_hours)
            )
        ...
        try:
            ...
            for file_path_str, result_data in cache_data.items():
                try:
                    ...
                    logger.warning(f"Failed to load cache entry for {file_path_str}: {e}")
            ...
            logger.error(f"Failed to load probe cache from {self.cache_file}: {e}")
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to save probe cache to {self.cache_file}: {e}")
